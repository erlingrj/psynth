...
void _buffer_destructor(void *array, bool tdomain) {
  ...
  if (idx >= NUM_BUFFERS) {
    lf_print_error_and_exit(
        "ERROR Tried to free something that wasnt allocated");
  }
  if (tdomain) {
    if (!_used[idx]) {
      lf_print_error_and_exit(
...
fsample_buffer_t *fsample_buffer_ctor() { return _buffer_ctor(false); }
sample_buffer_t *sample_buffer_ctor() { return _buffer_ctor(true); }
void fsample_buffer_destructor(void *array) {
  _buffer_destructor(array, false);
}
void sample_buffer_destructor(void *array) { _buffer_destructor(array, true); }
